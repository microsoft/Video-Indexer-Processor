...
class TimeParser:
    def __init__(self):
        ...
        if interval is not None and interval != "":
            self.interval_in_milliseconds = int(getenv("MILLISECONDS_INTERVAL"))
        else:
            # Set default
    ...
    @staticmethod
    def string_time_to_seconds(string_time):
        """
        This method converts string time to seconds
        it uses regex to handle both of the following  cases formats:
        - %H:%M:%S
        - %H:%M:%S.%f
        :param string_time:
        :return: passed time converted to seconds
        """
        ...
        return time_delta.total_seconds()  # seconds

    @staticmethod
    def seconds_to_time_string(seconds: int) -> str:
        """
        Convert seconds into a HH:MM:SS representation
        Args:
            seconds (int): numerical seconds

        Returns:
            str: string representation for seconds as HH:MM:SS
        """
